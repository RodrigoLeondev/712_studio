...
export type ScrollTiltedGridProps = {
  ...
  /**
   * Render arbitrary content per tile instead of a background image. When
   * provided, takes precedence over `images` — each node is placed inside the
   * tilting tile so cards/blocks ride the same scroll effect.
   */
...
export function ScrollTiltedGrid({
  ...
  const base = tiles ?? images;
  const items = useMemo(
    () =>
      loop ? Array.from({ length: cycles }, () => base).flat() : [...base],
    [loop, cycles, base],
  );
  ...
  const gridClass = [
    "mx-auto grid w-full grid-cols-1 sm:grid-cols-2 px-6",
